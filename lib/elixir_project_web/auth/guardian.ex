defmodule ElixirProjectWeb.Auth.Guardian do
  use Guardian, otp_app: :elixir_project
  ...
  def authenticate(token) do
    with {:ok, claims} <- decode_and_verify(token),
         {:ok, account} <- resource_from_token(claims),
  ...
  defp token_options(type) do
    case type do
      ...
      :reset ->
        [token_type: "reset", ttl: {15, :minute}]

      :admin ->
        [token_type: "admin", ttl: {10, :day}]
    end
  end
